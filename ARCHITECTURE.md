...
→ `exhibits/dedupe_integrity.py`, `exhibits/ranking_regression_check.py`
...
stays silent when they pass: that every duplicate cluster still leaves one row
standing, that the ledger and the CRM still agree on who exists, and that the six
companies the client confirmed are still on the list. A pre-commit hook blocks
credentials and private names before they can reach a repository.
...
→ `exhibits/ranking_regression_check.py`
